...
/**
    * 是这一年的第几天.
*/
inline int GetDate(int y,int d,int m){
...
bool CLasOperator::saveLasFile(const char *file_name,
	...
	sprintf(publicHeader.systemID,"xmu");
	sprintf(publicHeader.GenSoft,"XMU");

	//获取当地的时间。
	SYSTEMTIME stLocal;
	::GetLocalTime(&stLocal);
	publicHeader.creationDay =GetDate(stLocal.wYear,stLocal.wMonth, stLocal.wDay);
	publicHeader.creationYear =stLocal.wYear;  
	...
	publicHeader.maxX = (*points)[0].x;
	publicHeader.minX = (*points)[0].x;
	publicHeader.maxY = (*points)[0].y;
	publicHeader.minY = (*points)[0].y;
	publicHeader.maxZ = (*points)[0].z;
	publicHeader.minZ = (*points)[0].z; 
	publicHeader.xScale = 1.0e-4;
	publicHeader.yScale = 1.0e-4;
	publicHeader.zScale = 1.0e-4;
	...
	for(size_t i=0;i<points->size();++i){
		...
		pointRecord.intensity = unsigned short((*points)[i].classification);
		pointRecord.mask = 0;
		...
		pointRecord.GPS = 0.0;
		pointRecord.red =unsigned short(0);
		pointRecord.green =unsigned short(0);
		pointRecord.blue =unsigned short(0);
		lasFile.write((char *)&pointRecord,sizeof(PointDataRecord));
	}
	lasFile.flush(); 
	lasFile.clear();
	lasFile.seekp(0, ios::beg); // 指向begin处  
	lasFile.write((char *)(&publicHeader),sizeof(PublicHeaderBlock));
...
/*======================================
Readpointwithintensity函数简介
输入：const std::string name 加后缀的文件名
      std::vector<Point3D> & pointall las格式的点云
	  std::vector<unsigned short> & intenvec 存储强度用的向量
	  std::vector<RGBColor> & rgbvec 存储颜色RGB的向量
输出：分离强度和颜色信息，并读取点云
=========================================*/
//void Readpointwithintensity(const std::string name,
//	std::vector<Point3D> & pointall,std::vector<unsigned short> & intenvec,
//	std::vector<RGBColor> & rgbvec){
void Readpointwithintensity(const std::string name,
	std::vector<Point3D> & pointall,std::vector<unsigned short> & intenvec){
	//读取点云las文件
	CLasOperator lasptr;
	...
	if(!lasptr.readLasFile(name.c_str())){
		std::cout<<"Read file failed!"<<std::endl;
		std::cout<<"请确保所输入的文件存在于目录中!"<<std::endl;
	exit(0);
	}
	...
	//读取point3d下的三维坐标值
	//std::vector<Point3D> point3d;
	pointall=lasptr.getPointData();
	//合二为一

	for(size_t i=0;i!=pointall.size();i++){
		intenvec.push_back((unsigned short)pointrecords[i].intensity);
		/*RGBColor rxgybz;
		rxgybz.red=pointrecords[i].red;
		rxgybz.green=pointrecords[i].green;
		rxgybz.blue=pointrecords[i].blue;
		rgbvec.push_back(rxgybz);*/
	}
	std::cout<<"finish reading."<<std::endl;
}