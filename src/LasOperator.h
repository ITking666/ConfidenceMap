...
three element in RGB color space
===================================================================*/
struct RGBColor{
...
public:
	...
	bool saveLasFile(const char *file_name,
		std::vector<Point3D> *points); // Save Las file from points
...
//读取带强度和颜色的las文件
//void Readpointwithintensity(const std::string,std::vector<Point3D> &,std::vector<unsigned short> &,std::vector<RGBColor> & );
void Readpointwithintensity(const std::string,std::vector<Point3D> &,std::vector<unsigned short> &);
//las和pcd互转的函数
