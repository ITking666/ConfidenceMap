...
struct heapNode
{
	float lowerCost;      //当前节点往后排列，整个回路所能取得的总耗费的下限
	float currentCost;    //从根到当前节点的当前排列的耗费
	float restMinCost;    //从当前节点到最后一个节点，每个节点的最小出边的耗费之和
...
private:
	...
	//全局变量
	int iNodeNum;
	//vEffective[1:n][1:n] 表示城市之间的旅行耗费，不存在路径则耗费等于-1
